...
if (empty($md5)) {
    exitFail("Nie przeslano sumy kontrolnej.", true);
}
if (empty($timestamp)) {
    $timestamp = time();
}
...
if (!empty($action)) {
    ...
    // MD5 verification
    $md5ControlSum = generateMD5Sum($galaxySalt, $username, $timestamp, $passwd, $action, $email);

    if ($md5ControlSum != $md5) {
        // not valid MD5 sum
        exitFail("Niezgodna suma kontrolna.", true);
    } else {
        ...
        if ($action == "ENABLE") {
            ...
            if ($numberOfUsersFound == 0) {
                ...
                $queryAdd = sprintf("INSERT INTO galaxy_user(create_time, update_time, email, password, "
                        ...
                        . "'%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');", date('Y-m-d H:i:s', $timestamp), date('Y-m-d H:i:s', $timestamp), pg_escape_string($email), $passwd, 'f', 'f', 'f', pg_escape_string($username), 't'
                );
                ...
                if ($record["email"] != $email) {
                    // email change
                    $queryUpdate = sprintf("UPDATE galaxy_user SET update_time='%s', email='%s' WHERE username='%s';", date('Y-m-d H:i:s', $timestamp), pg_escape_string($email), pg_escape_string($username)
                    );
                ...
                if ($record["password"] != $passwd) {
                    // password change
                    $queryUpdate = sprintf("UPDATE galaxy_user SET update_time='%s', password='%s' WHERE username='%s';", date('Y-m-d H:i:s', $timestamp), $passwd, pg_escape_string($username)
                    );
                    $resultUpdate = pg_query($queryUpdate);
                ...
                if ($record["deleted"] == "t") {
                    // activate account
                    $queryUpdate = sprintf("UPDATE galaxy_user SET update_time='%s', deleted='%s' WHERE username='%s';", date('Y-m-d H:i:s', $timestamp), "f", pg_escape_string($username)
                    );
                    $resultUpdate = pg_query($queryUpdate);
            ...
            if ($numberOfUsersFound == 0) {
                ...
                if ($record["deleted"] == "f") {
                    $queryUpdate = sprintf("UPDATE galaxy_user SET update_time='%s', deleted='%s' WHERE username='%s';", date('Y-m-d H:i:s', $timestamp), "t", pg_escape_string($username)
                    );
                    $resultUpdate = pg_query($queryUpdate);
